...
import { useMyTasks, useMyJobs, useMyDashboardStats } from '../hooks/useData';
import {
  CheckSquare,
  ClipboardList,
  AlertTriangle,
...
export default function Dashboard() {
  ...
  const { data: stats } = useMyDashboardStats(profile?.id || null);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          ...
          trend={stats?.overdue_tasks ? `${stats.overdue_tasks} overdue` : undefined}
          trendType={stats?.overdue_tasks ? 'warning' : 'neutral'}
        />
        <StatCard
          icon={<ClipboardList className="text-blue-400" />}
          label="Active Jobs"
          value={stats?.active_jobs ?? jobs.length}
        />
        <StatCard
        ...
        <StatCard
          ...
          value={stats?.jobs_completed_7d ?? 0}
        />
      </div>
      ...
      {profile?.staff_role && ['admin', 'management', 'scheduler', 'sales'].includes(profile.staff_role) && (
